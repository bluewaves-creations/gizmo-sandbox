...
4. Provision the Container, the `Sandbox` and `WarmPool` Durable Objects, and the cron trigger.
5. Build the Docker image (~3 minutes the first time — the skill stack pre-installs ~15 Python packages including `reportlab`, `xhtml2pdf`, `pyhanko`, `matplotlib`, `ebooklib`).
6. Ask you to set the `SANDBOX_API_KEY` secret — **paste your AI Gateway token here**, the same value Gizmo already has in Keychain.
7. Print your bridge URL (`https://gizmo-sandbox-bridge-{xxx}.{your-subdomain}.workers.dev`).

Copy that URL into **Gizmo → Settings → Skill Sandbox → Bridge URL**, tap **Test connection**, and you're done.
